...
export function DeadlineItem({ prazo }: DeadlineItemProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative flex flex-col justify-between bg-white dark:bg-[#1C1C1E] border border-black/[0.06] dark:border-white/[0.06] rounded-[10px] mb-2 hover:border-blue-500/20 transition-all group",
        isMobile ? "min-h-[64px] px-2.5 py-2" : "min-h-[88px] px-4 py-3.5"
      )}
    >
      {/* Row 1: Header Info */}
      <div className={cn("flex flex-wrap items-center gap-1", isMobile ? "mb-0.5" : "mb-1")}>
        <Badge variant="unstyled" className={cn("py-0.5 text-[10px] sm:text-[12px] font-bold uppercase tracking-wide h-5 flex items-center gap-1 shrink-0", isMobile ? "px-1.5" : "px-2", badgeVariant)}>
          {statusIcon}
          {statusText}
        </Badge>
        ...
        <div className="flex items-center gap-2 min-w-0">
            ...
            <h3 className={cn("font-bold text-[#1D1D1F] dark:text-white truncate", isMobile ? "text-[12px]" : "text-[15px]")}>
            {prazo.descricao}
            </h3>
        </div>
      </div>
      ...
      <div className={cn("flex flex-col gap-1.5 sm:gap-2 sm:flex-row sm:items-center sm:justify-between mt-auto border-t border-dashed border-gray-100 dark:border-gray-800", isMobile ? "pt-1" : "pt-1.5")}>
         <div className="flex items-center gap-2.5">
            <Avatar className={cn("ring-1 ring-white dark:ring-[#1C1C1E]", isMobile ? "w-5 h-5" : "w-7 h-7")}>
            ...
            <div className="flex flex-col">
                <span className={cn("text-[#1D1D1F] dark:text-white font-medium truncate leading-tight", isMobile ? "text-[11px] max-w-[96px]" : "text-[12px] max-w-[140px]")}>
                {prazo.responsavel}
                </span>
            </div>
         </div>
         ...
         <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            <span className={cn("text-[#6E6E73] font-medium flex items-center gap-1", isMobile ? "text-[10px]" : "text-[12px]")}>
               <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#86868B]" />
               {format(dueDate, "dd/MM")}
            </span>

            {/* Progress Bar */}
             <div className={cn("flex items-center gap-1.5", isMobile ? "w-[74px]" : "w-[100px]")}>
                ...
                <span className={cn("font-medium text-[#86868B] text-right", isMobile ? "text-[9px] w-6" : "text-[11px] w-8")}>{Math.round(progress)}%</span>
             </div>
         </div>
      </div>
    </motion.div>
